...
class ActiveSpeakerSubscriptionSet: SubscriptionSet {
    ...
    private var audioMediaObjects: [UInt16: OpusHandler] = [:]

    init(engine: DecimusAudioEngine, jitterDepth: TimeInterval, jitterMax: TimeInterval, opusWindowSize: OpusWindowSize) {
        self.engine = engine
    ...
    func receivedObject(headers: QObjectHeaders, data: Data, extensions: [NSNumber: Data]?) {
        ...
        guard let extensions = extensions,
              let clientIdExtension = extensions[OpusPublication.endpointIdKey] else {
            self.logger.error("Missing expected client ID extension")
            return
        }

        // Parse.
        let clientId = clientIdExtension.withUnsafeBytes { $0.loadUnaligned(as: UInt16.self) }

        // Look up the media object for this client, or create one.
        let media: OpusHandler
        if let existing = self.audioMediaObjects[clientId] {
            media = existing
        } else {
            do {
                media = try OpusHandler(sourceId: String(clientId),
                                        engine: self.engine,
                                        measurement: nil,
