...
final class TestQHelpers: XCTestCase {
    func testSequentialObjectBlockingNameGate() throws {
        ...
        XCTAssert(namegate.handle(groupId: random(),
                                  objectId: 0,
                                  lastGroup: nil,
                                  lastObject: nil))

        // Non 0 object ID with no history should not go through.
        XCTAssertFalse(namegate.handle(groupId: random(),
        ...
        do {
            let currentGroup: UInt32 = random()
        ...
        do {
            ...
            XCTAssert(namegate.handle(groupId: nextGroup,
                                      objectId: 0,
                                      lastGroup: currentGroup,
                                      lastObject: 0))

            // Even when it jumps non-sequentially.
            XCTAssert(namegate.handle(groupId: nextGroup + 1,
                                      objectId: 0,
                                      lastGroup: currentGroup,
                                      lastObject: 0))
    ...
    func testAllowAllNameGate() {
        XCTAssertTrue(AllowAllNameGate().handle(groupId: random(),
                                                objectId: random(),
                                                lastGroup: random(),
                                                lastObject: random()))
    }

    private func random<T: FixedWidthInteger>() -> T {
