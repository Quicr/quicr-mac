...
@Test("Test Sliding Windows - Latest")
func testSlidingWindowLatest() throws {
    let length: TimeInterval = 10
    let window = SlidingTimeWindow<Int>(length: length)
    let start = Date.now
    window.add(timestamp: start.addingTimeInterval(-1), value: 1)
    window.add(timestamp: start, value: 2)
    try #require(window.latest() != nil)
    #expect(window.latest()!.0 == start)
}

@Test("Test Sliding Windows - Clear")
func testSlidingWindowClear() {
    let length: TimeInterval = 10
    let window = SlidingTimeWindow<Int>(length: length)
    let start = Date.now
    window.add(timestamp: start.addingTimeInterval(-1), value: 1)
